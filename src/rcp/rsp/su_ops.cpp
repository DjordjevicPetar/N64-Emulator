...
u8 LB(RSP& rsp, const RSPInstruction& instr)
{
    ...
    u32 rs = rsp.su().read_gpr(instr.i_type.rs);
    u32 address = rs + imm;
    u8 value = rsp.read<u8>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS);
...
u8 LH(RSP& rsp, const RSPInstruction& instr)
{
    ...
    u32 rs = rsp.su().read_gpr(instr.i_type.rs);
    u32 address = rs + imm;
    u16 value = rsp.read<u16>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS);
...
u8 LW(RSP& rsp, const RSPInstruction& instr)
{
    ...
    u32 rs = rsp.su().read_gpr(instr.i_type.rs);
    u32 address = rs + imm;
    u32 value = rsp.read<u32>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS);
...
u8 SB(RSP& rsp, const RSPInstruction& instr)
{
    ...
    rsp.write<u8>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS, value);
    return 1;
}
...
u8 SH(RSP& rsp, const RSPInstruction& instr)
{
    ...
    rsp.write<u16>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS, value);
    return 1;
}
...
u8 SW(RSP& rsp, const RSPInstruction& instr)
{
    ...
    rsp.write<u32>((address & 0x00000FFF) + RSP_DATA_MEMORY_START_ADDRESS, value);
    return 1;
}
